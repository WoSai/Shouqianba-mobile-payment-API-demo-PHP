...
function activate($vendor_sn, $vendor_key)
{
    ...
    $params['app_id'] = '2019******001786';    //app id，从服务商平台获取2017112500000439
    $params['code'] = '74649220';              //激活码内容11654978
    $params['device_id'] = '5210000010011';                 //设备唯一身份ID
...
function checkin($terminal_sn, $terminal_key)
{
    ...
    $params['device_id'] = '2019-07-03 09:49';                //设备唯一身份ID
...
function precreate($terminal_sn, $terminal_key)
{
    ...
    $params['client_sn'] = '1231ksjljkj';  //商户系统订单号,必须在商户系统内唯一；且长度不超过64字节
    $params['total_amount'] = '1';             //交易总金额
    $params['payway']='1';
    $params['subject'] = 'pizza';              //本次交易的概述
    $params['operator'] = 'Obama';             //发起本次交易的操作员
    $params['sub_payway']='6';           //内容为数字的字符串，如果要使用WAP支付，则必须传 "3", 使用小程序支付请传"4"
    //$params['payer_uid']='kun565045067';          //消费者在支付通道的唯一id,微信WAP支付必须传open_id,支付宝WAP支付必传用户授权的userId
    ...
    //$params['notify_url']='www.baidu.com';     //支付回调的地址
    $ret = pre_do_execute($params, $url, $terminal_sn, $terminal_key);
    return $ret ;
}
...
function  pay($terminal_sn, $terminal_key)
{
    ...
    $params['total_amount'] = '10000';                      //交易总金额,以分为单位
    //$params['payway'] = '1';                          //支付方式,1:支付宝 3:微信 4:百付宝 5:京东钱包
    $params['dynamic_id'] = '';       //条码内容（支付包或微信条码号）
    $params['subject'] = 'pizz';                        //交易简介
    $params['operator'] = 'kun';                        //门店操作员
...
function refund($terminal_sn, $terminal_key)
{
    ...
    $params['terminal_sn'] = $terminal_sn;       //收钱吧终端ID
    $params['sn'] = '7895239974544350';        //收钱吧系统内部唯一订单号（N）
    //$params['client_sn']='Se12345678';   //商户系统订单号,必须在商户系统内唯一；且长度不超过64字节
    //$params['client_tsn']='S12345676';  //商户退款流水号一笔订单多次退款，需要传入不同的退款流水号来区分退款，如果退款请求超时，需要发起查询，并根据查询结果的client_tsn判断本次退款请求是否成功
    $params['refund_amount'] = '10000';              //退款金额
    $params['refund_request_no'] = '001';        //商户退款所需序列号,表明是第几次退款(正常情况不可重复，意外状况爆出不变)
    $params['operator'] = 'kun';                 //门店操作员
...
function query($terminal_sn, $terminal_key)
{
    ...
    $params['terminal_sn'] = $terminal_sn;      //收钱吧终端ID
    $params['sn']='7895239328199504';         //收钱吧系统内部唯一订单号
    //$params['client_sn'] = '652110026320';    //商户系统订单号,必须在商户系统内唯一；且长度不超过64字节
...
{
    $params['terminal_sn'] = $terminal_sn;     //收钱吧终端ID
    $params['client_sn'] = '932409283402384';    //商户系统订单号,必须在商户系统内唯一；且长度不超过64字节
    $params['total_amount'] = '1';             //以分为单位,不超过10位纯数字字符串,超过1亿元的收款请使用银行转账
    $params['subject'] = '披萨';              //本次交易的概述
    //$params['payway']='1';
    $params['notify_url'] = 'www.shouqianba.com';   //支付回调的地址
    $params['operator'] = 'Obama';                    //发起本次交易的操作员
    $params['return_url'] = "http://www.baidu.com";  //处理完请求后，当前页面自动跳转到商户网站里指定页面的http路径
